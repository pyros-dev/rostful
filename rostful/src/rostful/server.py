...
class RostfulApp:
	...
	def add_service(self, service_name, ws_name=None, service_type=None):
		...
		#Should be implemented in RostfulBack and RostfulFront
	...
	def dispatch_request(self, request, start_response):
		return Response('DONT USE ME DIRECTLY FOR NOW')
...
"""
The ROSTful WSGI app.
"""
class RostfulBack(RostfulApp):

	def __init__(self):
		RostfulApp.__init__(self)
...
"""
The WSGI App delivering Javascript client code for the ROSTful server
"""
class RostfulFront(RostfulApp):
	...
	def add_service(self, service_name, ws_name=None, service_type=None):
		...
		#self.services[ws_name] = ServiceFront(service_name, service_type)
		return True
	...
	def add_topic(self, topic_name, ws_name=None, topic_type=None, allow_pub=True, allow_sub=True):
		...
		#self.topics[ws_name] = TopicFront(topic_name, topic_type, allow_pub=allow_pub, allow_sub=allow_sub)
		return True
	...
	def add_action(self, action_name, ws_name=None, action_type=None):
		...
		#self.actions[ws_name] = ActionFront(action_name, action_type)
		return True
		
	def dispatch_request(self, request, start_response):
		return Response('Rostful Client code!')
...
def servermain(with_static=True):
	...
	try:
		...
		rostfront = RostfulFront()		
		rostback = RostfulBack()
		
		rostback.add_services(args.services)
		rostfront.add_services(args.services)
		rostback.add_topics(args.topics)
		rostfront.add_topics(args.topics)
		rostback.add_topics(args.publishes, allow_pub=False)
		rostfront.add_topics(args.publishes, allow_pub=False)
		rostback.add_topics(args.subscribes, allow_sub=False)
		rostfront.add_topics(args.subscribes, allow_sub=False)
		rostback.add_actions(args.actions)
		rostfront.add_actions(args.actions)

		server = DispatcherMiddleware(rostfront, {
			'/ros': rostback
		})
